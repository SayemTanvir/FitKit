...
export function ProgrammeCatalog() {
  ...
  return <div className="space-y-6">
    <div className="glass rounded-2xl p-6 sm:p-8 flex flex-wrap items-center justify-between gap-4"><div><h1 className="font-display text-2xl font-semibold text-white">{admin ? 'Programme Studio' : 'Explore Programmes'}</h1><p className="text-sm text-slate-400 mt-1">{admin ? 'Create, preview and publish complete training programmes.' : 'Find a structured training programme that fits your goals.'}</p></div>{admin && <Link className="btn-primary rounded-xl px-4 py-2.5 text-sm font-semibold inline-flex items-center gap-2" to="/programmes/new"><Plus className="w-4 h-4" /> New programme</Link>}</div>
